...
#	cmake -DANDROID_API=$(ANDROID_API) -DANDROID_PLATFORM=android-$(ANDROID_API) \
	-DCMAKE_C_STANDARD_LIBRARIES="-lm -latomic -llog" -DCMAKE_VERBOSE_MAKEFILE=on \
	-DCMAKE_TOOLCHAIN_FILE=$(NDKDIR)/build/cmake/android.toolchain.cmake \
	-DDUCKDB_EXTRA_LINK_FLAGS="-llog -lz" \
	-DOPENSSL_ROOT_DIR=$(OPENSSL_ROOT_DIR) \
    -DOPENSSL_CRYPTO_LIBRARY=$(OPENSSL_CRYPTO_LIBRARY) \
    -DOPENSSL_SSL_LIBRARY=$(OPENSSL_SSL_LIBRARY) \
    -DOPENSSL_INCLUDE_DIR=$(OPENSSL_INCLUDE_DIR) \
	-DANDROID_ABI=$(ANDROID_ABI) -DDUCKDB_PLATFORM=$(DUCKDB_PLATFORM) \

release: ${EXTENSION_CONFIG_STEP}
	mkdir -p ./build/release_$(DUCKDB_PLATFORM) &&\
	cd build/release_$(DUCKDB_PLATFORM) && \
	cmake -DBUILD_UNITTESTS=false \
	-DANDROID_ABI=$(ANDROID_ABI) -DANDROID_API=$(ANDROID_API) -DANDROID_PLATFORM=android-$(ANDROID_API) \
	-DCMAKE_VERBOSE_MAKEFILE=on \
    -DCMAKE_TOOLCHAIN_FILE=$(ANDROID_NDK)/build/cmake/android.toolchain.cmake \
    -DDUCKDB_EXTRA_LINK_FLAGS="-llog -lz -lc++" \
	$(GENERATOR) $(FORCE_COLOR) ${WARNINGS_AS_ERRORS} ${FORCE_WARN_UNUSED_FLAG} ${FORCE_32_BIT_FLAG} ${DISABLE_UNITY_FLAG} ${DISABLE_SANITIZER_FLAG} ${STATIC_LIBCPP} ${CMAKE_VARS} ${CMAKE_VARS_BUILD} -DCMAKE_BUILD_TYPE=Release ../.. && \
	cmake --build . --config Release

release_original: ${EXTENSION_CONFIG_STEP}
